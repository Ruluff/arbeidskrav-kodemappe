  ...
  /*
  = ["Dragon", "Monster", "Swamp Monster"];
  const enemyImage = ["dragon.jpg", "monster.jpg", "swamp-monster.jpg"];

  const enemyHp = (min, max) => {
    return Math.random() * (max - min) + min;
  };

  const enemyDamage = (min, max) => {
    return Math.random() * (max - min) + min;
  };

  const enemyCharacter = {enemyName, enemyImage, enemyHp, enemyDamage};
